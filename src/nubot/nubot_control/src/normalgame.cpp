...
*/
    ...
    if(selected_action_ == Actions::StaticPass) //到射门位置 如果对方机器人堵住门就转向
    {
        ...
        br = DPoint(FIELD_LENGTH*1.0/2, (rand_num_+5)*1.0) - robot_pos_;
        m_plan_->positionAvoidObs2(br.angle().radian_);
...
//    }
    ...
    if(selected_action_ == Catch_Positioned) //防守，对方将要射门
    {
        ...
        m_plan_->move2Positionwithobs_noball(target);
        m_plan_->positionAvoidObs(DPoint(0.0, robot_pos_.y_));
...
{/*
    opp_robot_ = world_model_->Opponents_[0];
    DPoint target, br;
    if(world_model_->BallInfo_[world_model_->AgentID_-1].isLocationKnown() == false)
    {
        selected_action_ = CanNotSeeBall;
    ...
    {
        target = ball_pos_ + opp_robot_[0];
        target.x_ /= 2.0;
        target.y_ /= 2.0;
        if(ball_pos_.distance(opp_robot_[0]) < LIMITDRIBLLEDIS && ball_pos_.distance(opp_robot_[0]) < ball_pos_.distance(world_model_->RobotInfo_[world_model_->pass_ID_-1].getLocation())) //对方带球////
        {
            selected_action_ = CanNotSeeBall;
        }
        else if(world_model_->IsOurDribble_) //我方带球
        {
            selected_action_ = Positioned;
        }
        else
    ...
    {
        ROS_INFO("CanNotSeeBall");
        target = DPoint((-FIELD_LENGTH*1.0/2+opp_robot_[0].x_)/2,opp_robot_[0].y_/2);
        br = opp_robot_[0] - robot_pos_;
